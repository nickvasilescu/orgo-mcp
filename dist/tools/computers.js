    ...
    registerOrgoTool(server, {
        ...
        handler: async ({ workspace_id, compact, limit }) => {
            try {
                ...
                if (limit === undefined) {
                    payload.count = computers.length;
                }
                return {
                    content: [{ type: "text", text: compact ? jsonTextCompact(payload) : jsonText(payload) }],
                };
            }
            catch (e) {
        ...
        handler: async ({ workspace, workspace_id, name, os, ram, cpu, gpu, resolution, image }) => {
            try {
                ...
                if (!wsId) {
                    ...
                    try {
                        const found = (await apiRequest("GET", `projects/by-name/${encodeURIComponent(workspace)}`, apiKey));
                        wsId = found.id || found.project?.id;
                    }
                    catch (e) {
                        if (e instanceof HttpError && e.status === 404) {
                            const made = (await apiRequest("POST", "projects", apiKey, { json: { name: workspace } }));
                            wsId = made.id || made.project?.id;
                        }
                        else {
                            throw e;
                        }
                    ...
                    if (!wsId)
                        throw new Error(`Could not resolve or create workspace "${workspace}".`);
                }
    ...
    registerOrgoTool(server, {
        ...
        handler: async ({ computer_id, compact }) => {
            try {
                ...
                return { content: [{ type: "text", text: compact ? jsonTextCompact(data) : jsonText(data) }] };
            }
            catch (e) {
                return { content: [{ type: "text", text: handleError(e) }], isError: true };
            }
        ...
        handler: async ({ computer_id, name, workspace_id }) => {
            try {
                ...
                if (name)
                    body.name = name;
                if (workspace_id)
                    body.targetProjectId = workspace_id;
    ...
    registerOrgoTool(server, {
        ...
        handler: async ({ computer_id, cpu, ram, disk_size_gb, bandwidth_limit_mbps }) => {
            try {
                ...
                if (ram !== undefined)
                    body.mem_gb = ram;
                if (disk_size_gb !== undefined)
                    body.disk_size_gb = disk_size_gb;
                if (bandwidth_limit_mbps !== undefined)
                    body.bandwidth_limit_mbps = bandwidth_limit_mbps;
