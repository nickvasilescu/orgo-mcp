...
function tenantKey(apiKey, computerId) {
...
const instanceDetailsCache = new Map();
 ...
 */
async function apiRequest(method, path, apiKey, options = {}) {
 ...
 */
async function getVmAuthToken(computerId, apiKey) {
    const cacheKey = tenantKey(apiKey, computerId);
    const cached = vmTokenCache.get(cacheKey);
    if (cached)
        return cached;
    const data = (await apiRequest("GET", `computers/${computerId}/vnc-password`, apiKey, { timeout: 15000 }));
 ...
 */
async function getInstanceInfo(computerId, apiKey) {
    const cacheKey = tenantKey(apiKey, computerId);
    const cached = instanceDetailsCache.get(cacheKey);
    if (cached && cached.expires > Date.now())
        return cached.details;
    const info = (await apiRequest("POST", `computers/${computerId}/ensure-running`, apiKey, { timeout: 15000 }));
 ...
 */
async function getVmEndpoint(computerId, apiKey) {
 ...
 */
async function directVmRequest(method, computerId, endpoint, apiKey, options = {}) {
 ...
 */
async function computerAction(method, computerId, endpoint, apiKey, options = {}) {
 ...
 */
async function resolveFlyInstanceId(computerId, apiKey) {
 ...
 */
async function uploadFile(apiKey, workspaceId, filename, fileBytes, options = {}) {
 ...
 */
function clearVncCache(computerId) {
    if (computerId) {
